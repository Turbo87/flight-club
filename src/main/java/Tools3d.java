...
public class Tools3d
{
	...
	static Vector square(int face, float bottom, float left, float top, float right, float d)
	{
		...
		{
		case YZF:
			ps[0].y = left;ps[0].z = bottom;
			ps[1].y = left;ps[1].z = top;
			ps[2].y = right;ps[2].z = top;
			ps[3].y = right;ps[3].z = bottom;
			break;
			
		case YZB:
			ps[0].y = left;ps[0].z = bottom;
			ps[3].y = left;ps[3].z = top;
			ps[2].y = right;ps[2].z = top;
			ps[1].y = right;ps[1].z = bottom;
			break;

		case ZXB:
			ps[0].x = left;ps[0].z = bottom;
			ps[1].x = left;ps[1].z = top;
			ps[2].x = right;ps[2].z = top;
			ps[3].x = right;ps[3].z = bottom;
			break;
			
		case ZXF:
			ps[0].x = left;ps[0].z = bottom;
			ps[3].x = left;ps[3].z = top;
			ps[2].x = right;ps[2].z = top;
			ps[1].x = right;ps[1].z = bottom;
			break;

		case XYF:
			ps[0].x = left;ps[0].y = bottom;
			ps[1].x = right;ps[1].y = bottom;
			ps[2].x = right;ps[2].y = top;
			ps[3].x = left;ps[3].y = top;
			break;
	...
	static Object3d unitCube(ModelViewer theApp, boolean isSolid)
	{
		...
		cube.addWire(unitSquare(XYF, d),c,isSolid);c = Color.red;
		cube.addWire(unitSquare(XYB, -d),c,isSolid);c = Color.blue;
		
		cube.addWire(unitSquare(YZF, d),c,isSolid);c = Color.magenta;
		cube.addWire(unitSquare(YZB, -d),c,isSolid);c = Color.orange;
		
		cube.addWire(unitSquare(ZXF, d),c,isSolid);c = Color.pink;
		cube.addWire(unitSquare(ZXB, -d),c,isSolid);c = Color.yellow;
