...
class CameraMan implements EventInterface
{
	...
	if (inMode == WATCH_PLAN) {
		...
		focus = subject1.getFocus(); focus.x = 0; focus.y += PLAN_Y_OFFSET;
		eye = new Vector3d(Landscape.TILE_WIDTH/2,focus.y,PLAN_H);
		cameraSubject = subject1;
	    } else if (subject2!=null) {
		focus = subject2.getFocus(); focus.x = 0; focus.y += PLAN_Y_OFFSET;
		eye = new Vector3d(Landscape.TILE_WIDTH/2,focus.y,PLAN_H);
		cameraSubject = subject2;
	    } else  {
	...
	*/
	...
	if (mode != WATCH_PLAN) {
	    ...
	    Vector3d f = cameraSubject.getFocus();f.x = 0;f.y += PLAN_Y_OFFSET;
	    moveFocus(f);
	}
    }
	...
	*/
    ...
    Vector3d getFocus(){return new Vector3d(focus.x, focus.y, focus.z);}
    Vector3d getEye(){return new Vector3d(eye.x, eye.y, eye.z);}
